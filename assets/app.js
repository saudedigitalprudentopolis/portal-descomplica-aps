...
function monthlyKpis(section){
  ...
  return `<div class="monthly-kpis">
    <div class="monthly-kpi"><span>Média de março</span><strong>${formatMonthly(stats.previousAverage)}</strong><small>resultado anterior</small></div>
    <div class="monthly-kpi monthly-kpi-primary"><span>Média de abril</span><strong>${formatMonthly(stats.currentAverage)}</strong><small>resultado atual</small></div>
    <div class="monthly-kpi ${delta>0?'is-positive':delta<0?'is-negative':'is-neutral'}"><span>Evolução média</span><strong>${delta>0?'+':''}${formatMonthly(delta)}</strong><small>de março para abril</small></div>
...
function monthlyDashboard(section){
  ...
  const headers=['Posição','Unidade','Março','Abril','Evolução'];
  ...
  return `${monthlyKpis(alignedSection)}
    ...
    <div class="monthly-table-card card">
      <div class="monthly-table-head"><div><h3>Resultados por unidade</h3><p>Use as setas dos títulos para reorganizar a tabela.</p></div><div class="monthly-legend"><span><i class="legend-dot previous"></i>Março</span><span><i class="legend-dot current"></i>Abril</span><span><i class="legend-dot evolution"></i>Evolução</span></div></div>
