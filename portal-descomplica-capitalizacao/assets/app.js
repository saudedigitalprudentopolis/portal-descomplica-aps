...
const state = { data:null, route:'home', query:'', monthlySort:{section:0,column:0,direction:'asc'}, summarySort:{section:0,column:0,direction:'asc'}, detailGroup:'familia', detailIndicator:'C2', detailSort:{key:'team',direction:'asc'}, quadrSort:{group:0,column:0,direction:'asc'}, municipalSort:{column:6,direction:'desc'} };
...
const icons = {
  ...
  detalhamento: svgIcon('<path d="M4 3h16v18H4z"/><path d="M8 8h8"/><path d="M8 12h5"/><path d="M8 16h3"/>'),
...
const labels = {home:'Início', vinculo:'Vínculo e Acompanhamento', familia:'Saúde da Família', bucal:'Saúde Bucal', emulti:'eMulti', mensais:'Evolução Mensal', resumo:'Resumo das notas Mensal', detalhamento:'Detalhamento Mensal', quadrimestre:'Avaliação Quadrimestral', municipios:'Comparativo de Municípios', sobre:'Sobre'};
const navGroups = [
  ['Portal',['home']],
  ['Indicadores',['vinculo','familia','bucal','emulti']],
  ['Resultados',['mensais','resumo','detalhamento','quadrimestre','municipios']]
];
const aboutInfo = {
  ...
  apoio: [
    ...
    ['Maria Adriane Krevei', 'Coordenadora do Departamento de Saúde Bucal']
  ]
};
...
function monthlyPage(table){
  ...
  return `<div class="page-title monthly-page-title"><div><div class="eyebrow" style="color:var(--primary)">Resultados</div><h1>Evolução Mensal</h1></div></div>
...
function resumoPage(table){
  ...
  const section=sections[selected];
  return `<div class="page-title monthly-page-title"><div><div class="eyebrow" style="color:var(--primary)">Resultados</div><h1>Resumo das notas Mensal</h1><div class="page-period-badge">Competência Jul/26</div></div></div>
...
function quadrimestrePage(table){
  ...
  return `<div class="page-title monthly-page-title"><div><div class="eyebrow" style="color:var(--primary)">Resultados</div><h1>Avaliação Quadrimestral</h1><div class="page-period-badge">Indicadores do 1º quadrimestre de 2026</div></div></div>
...
function municipiosPage(table){
  ...
  const section=sections[selected];
  return `<div class="page-title monthly-page-title"><div><div class="eyebrow" style="color:var(--primary)">Resultados</div><h1>Comparativo de Municípios</h1><div class="page-period-badge">Indicadores do 1º quadrimestre de 2026</div></div></div>
...
/* Detalhamento do Indicador Mensal */
function detailMonthlyData(){
  return window.MONTHLY_DETAIL_DATA || null;
}
function detailPercentTone(value){
...
function detailCvatDimensionTone(value){
  const n=Number(value);
  if(n>85) return 'otimo';
  if(n>=65) return 'bom';
  if(n>=45) return 'suficiente';
  return 'regular';
}
function detailCvatTotalTone(value){
  const n=Number(value);
  if(n>8.5) return 'otimo';
  if(n>=7) return 'bom';
  if(n>=5) return 'suficiente';
  return 'regular';
}
function detailSpecialTone(code,value){
  const n=Number(value);
  if(code==='B1'){
    if(n<=0.25) return 'regular';
    if(n<=0.75) return 'suficiente';
...
function detailToneRank(tone){
  return ({regular:0,suficiente:1,bom:2,otimo:3})[tone] ?? 0;
}
function detailPill(value,max,tone){
  if(value===null || value===undefined || Number.isNaN(Number(value))){
    return '<span class="summary-pill neutral">—</span>';
  }
  return `<span class="summary-pill ${tone}">${formatMonthly(value)}${max!==null&&max!==undefined?`<small>/${formatMonthly(max)}</small>`:''}</span>`;
}

function detailRawLine(text,title=''){
  if(!text) return '';
  return `<div class="detail-raw"${title?` title="${esc(title)}"`:''}>${esc(text)}</div>`;
}
function detailInt(value){
  const n=Number(value);
  return Number.isFinite(n)?n.toLocaleString('pt-BR'):'—';
}

function detailSortHeader(label,key){
  const current=state.detailSort || {key:'team',direction:'asc'};
  const active=current.key===key;
  const direction=active?current.direction:'';
  return `<th><button class="municipal-sort-button ${active?'active':''}" data-detail-sort="${esc(key)}"><span>${esc(label)}</span><span class="municipal-sort-arrows" aria-hidden="true"><i class="${active&&direction==='asc'?'active':''}">▲</i><i class="${active&&direction==='desc'?'active':''}">▼</i></span></button></th>`;
}
function detailSortRows(rows,getter){
  const current=state.detailSort || {key:'team',direction:'asc'};
  const direction=current.direction==='desc'?-1:1;
  return [...rows].sort((a,b)=>{
    const av=getter(a,current.key);
    const bv=getter(b,current.key);
    if(current.key==='team'){
    ...
    if(an!==null && bn===null) return -1;
    if(an!==null && bn!==null && Math.abs(an-bn)>0.000001) return (an-bn)*direction;
    return String(a.team||'').localeCompare(String(b.team||''),'pt-BR',{sensitivity:'base'});
  });
}
function detailGroupFilters(){
  const selected=state.detailGroup||'familia';
  const groups=[['familia','Saúde da Família'],['bucal','Saúde Bucal'],['emulti','eMulti']];
  return `<div class="summary-choice-line">${groups.map(([key,label])=>`<button class="monthly-tab summary-choice-btn ${selected===key?'active':''}" data-detail-group="${key}"><strong>${esc(label)}</strong></button>`).join('')}</div>`;
}
function detailIndicatorFilters(){
  if((state.detailGroup||'familia')!=='familia') return '';
  const selected=state.detailIndicator||'C2';
  const items=[
  ...
  return `<div class="detail-indicator-line">${items.map(([key,label])=>`<button class="detail-indicator-btn ${selected===key?'active':''}" data-detail-indicator="${key}">${esc(label)}</button>`).join('')}</div>`;
}
function detailLegend(){
  return `<div class="monthly-legend summary-legend"><span><i class="legend-dot previous"></i>Regular</span><span><i class="legend-dot current"></i>Suficiente</span><span><i class="legend-dot bom"></i>Bom</span><span><i class="legend-dot otimo"></i>Ótimo</span></div>`;
}
function detailTable(title,headers,rowsHtml,colspan){
  return `<div class="monthly-table-card card summary-table-card detail-monthly-table-card"><div class="monthly-table-head"><div><h3>${esc(title)}</h3></div>${detailLegend()}</div><div class="monthly-table-wrap"><table class="monthly-table summary-table detail-monthly-table"><thead><tr>${headers}</tr></thead><tbody>${rowsHtml||`<tr><td colspan="${colspan}" class="empty">Nenhuma equipe encontrada.</td></tr>`}</tbody></table></div></div>`;
}
function detailFamilyIndicator(){
  const data=detailMonthlyData();
  const code=state.detailIndicator||'C2';
  if(!data?.data?.[code]) return '<div class="empty card">Dados não disponíveis para este indicador.</div>';

  if(code==='C1'){
    const rows=detailSortRows(data.data.C1,(row,key)=>key==='team'?row.team:key==='num'?row.num:key==='den'?row.den:row.total);
    const body=rows.map(row=>`<tr data-detail-row><td class="unit-name">${esc(row.team)}</td><td>${formatMonthly(row.num)}</td><td>${formatMonthly(row.den)}</td><td>${detailPill(row.total,null,detailC1Tone(row.total))}</td></tr>`).join('');
    const headers=`${detailSortHeader('Equipe','team')}${detailSortHeader('Demanda programada','num')}${detailSortHeader('Total de atendimentos','den')}${detailSortHeader('Resultado','total')}`;
    return detailTable('Mais Acesso',headers,body,4);
  }

  if(code==='CVAT'){
    const rows=detailSortRows(data.data.CVAT,(row,key)=>key==='team'?row.team:key==='cadastro'?row.cadastroScore:key==='acomp'?row.acompScore:row.total);
    const body=rows.map(row=>{
      const cadastroTone=detailCvatDimensionTone(row.cadastroPct);
      const acompTone=detailCvatDimensionTone(row.acompPct);
      const dimensions=[
        {label:'Cadastro',tone:cadastroTone,pct:row.cadastroPct},
        {label:'Acompanhamento',tone:acompTone,pct:row.acompPct}
      ].sort((a,b)=>detailToneRank(a.tone)-detailToneRank(b.tone) || a.pct-b.pct);
      const attention=dimensions[0].tone==='otimo'?'':dimensions[0].label;
      const cadastroRaw=`${detailInt(row.cadastroIndividual)} ind. + ${detailInt(row.cadastroCompleto)} completos • parâmetro ${detailInt(row.parametro)}`;
      const cadastroTitle=`Total com cadastro: ${detailInt(row.cadastroTotal)} • Vinculadas: ${detailInt(row.vinculadas)}`;
      const acompRaw=`${detailInt(row.acompanhadas)} acompanhadas • parâmetro ${detailInt(row.parametro)}`;
      const acompTitle=`Sem critério: ${detailInt(row.semCriterio)} • Crianças/idosos: ${detailInt(row.criancaIdoso)} • BPC/PBF: ${detailInt(row.bpcPbf)} • Dupla vulnerabilidade: ${detailInt(row.duplaVulnerabilidade)}`;
      return `<tr data-detail-row><td class="unit-name">${esc(row.team)}</td><td>${detailPill(row.cadastroScore,3,cadastroTone)}${detailRawLine(cadastroRaw,cadastroTitle)}</td><td>${detailPill(row.acompScore,7,acompTone)}${detailRawLine(acompRaw,acompTitle)}</td><td>${detailPill(row.total,10,detailCvatTotalTone(row.total))}</td><td class="detail-attention">${esc(attention)}</td></tr>`;
    }).join('');
    const headers=`${detailSortHeader('Equipe','team')}${detailSortHeader('Cadastro','cadastro')}${detailSortHeader('Acompanhamento','acomp')}${detailSortHeader('Total','total')}<th>Ponto de Atenção</th>`;
    return detailTable('Vínculo e Acompanhamento',headers,body,5);
  }

  const info=data.indicators?.[code];
  const practices=info?.practices||[];
  const rows=detailSortRows(data.data[code],(row,key)=>{
    if(key==='team') return row.team;
    if(key==='total') return row.total;
    if(/^item\d+$/.test(key)){
      const idx=Number(key.replace('item',''));
      return row.items?.[idx]?.score;
    }
    return row.team;
  });
  const headers=`${detailSortHeader('Equipe','team')}${practices.map((p,index)=>detailSortHeader(`${p[0]} · ${p[1]}`,`item${index}`)).join('')}${detailSortHeader('Total','total')}<th>Ponto de Atenção</th>`;
  const body=rows.map(row=>{
    const evaluated=(row.items||[]).map((item,index)=>({
      label:item.label || practices[index]?.[1] || `Item ${index+1}`,
      score:item.score,
      pct:item.pct,
      num:item.num,
      den:item.den,
      max:item.max ?? practices[index]?.[2] ?? null,
      tone:detailPercentTone(item.pct)
    }));
    const worst=[...evaluated].sort((a,b)=>detailToneRank(a.tone)-detailToneRank(b.tone) || a.pct-b.pct)[0];
    const attention=!worst || worst.tone==='otimo'?'':worst.label;
    return `<tr data-detail-row><td class="unit-name">${esc(row.team)}</td>${evaluated.map(item=>`<td>${detailPill(item.score,item.max,item.tone)}${detailRawLine(`${detailInt(item.num)} / ${detailInt(item.den)}`)}</td>`).join('')}<td>${detailPill(row.total,100,detailPercentTone(row.total))}</td><td class="detail-attention">${esc(attention)}</td></tr>`;
  }).join('');
  return detailTable(info?.title||code,headers,body,practices.length+3);
}
function detailSpecialGroup(group){
  const data=detailMonthlyData();
  const key=group==='bucal'?'ODONTO':'EMULTI';
  const info=data?.special?.[key];
  if(!info || !data?.data?.[key]) return '<div class="empty card">Dados não disponíveis.</div>';
  const metrics=info.metrics||[];
  const rows=detailSortRows(data.data[key],(row,sortKey)=>{
    if(sortKey==='team') return row.team;
    return row.metrics?.[sortKey]?.value;
  });
  const headers=`${detailSortHeader('Equipe','team')}${metrics.map(metric=>detailSortHeader(`${metric[0]} · ${metric[1]}`,metric[0])).join('')}<th>Ponto de Atenção</th>`;
  const body=rows.map(row=>{
    const evaluated=metrics.map(metric=>{
      const code=metric[0];
      const metricData=row.metrics?.[code] || {};
      const value=metricData.value;
      return {label:metric[1],value,num:metricData.num,den:metricData.den,tone:detailSpecialTone(code,value)};
    });
    const worst=[...evaluated].sort((a,b)=>detailToneRank(a.tone)-detailToneRank(b.tone) || Number(a.value)-Number(b.value))[0];
    const attention=!worst || worst.tone==='otimo'?'':worst.label;
    return `<tr data-detail-row><td class="unit-name">${esc(row.team)}</td>${evaluated.map(item=>`<td>${detailPill(item.value,null,item.tone)}${detailRawLine(`${detailInt(item.num)} / ${detailInt(item.den)}`)}</td>`).join('')}<td class="detail-attention">${esc(attention)}</td></tr>`;
  }).join('');
  return detailTable(info.title||'',headers,body,metrics.length+2);
}
function detailMonthlyDashboard(){
  const group=state.detailGroup||'familia';
  return group==='familia'?detailFamilyIndicator():detailSpecialGroup(group);
}
function detailMonthlyPage(){
  const data=detailMonthlyData();
  if(!data) return '<div class="empty card">Detalhamento mensal ainda não configurado.</div>';
  return `<div class="page-title monthly-page-title"><div><div class="eyebrow" style="color:var(--primary)">Resultados</div><h1>Detalhamento Mensal</h1><div class="page-period-badge">Competência ${esc(data.competencia||'Jul/26')}</div></div></div>
    <section class="monthly-selector card summary-selector-compact detail-monthly-selector">${detailGroupFilters()}${detailIndicatorFilters()}<div class="monthly-search detail-monthly-search"><span>⌕</span><input id="detailFilter" placeholder="Pesquisar unidade..." /></div></section>
    <div id="detailMonthlyDashboard">${detailMonthlyDashboard()}</div>`;
}

function classify(v){const n=Number(String(v).replace(',','.'));if(Number.isNaN(n))return '';return toneScale10(n)}
function tablePage(key){if(key==='detalhamento')return detailMonthlyPage();const t=state.data.tables[key];if(!t)return'<div class="empty card">Tabela ainda não configurada.</div>';if(key==='mensais')return monthlyPage(t);if(key==='resumo')return resumoPage(t);if(key==='quadrimestre')return quadrimestrePage(t);if(key==='municipios')return municipiosPage(t);const headers=t.headers||[];const rows=t.rows||[];return `<div class="page-title"><div><div class="eyebrow" style="color:var(--primary)">Resultados</div><h1>${esc(t.title||labels[key])}</h1><p>${esc(t.subtitle||'Dados exibidos diretamente da planilha oficial.')}</p></div></div><div class="filterbar"><input id="tableFilter" placeholder="Filtrar unidade ou município..." /></div><div class="card table-card"><table class="data-table"><thead><tr>${headers.map(h=>`<th>${esc(h)}</th>`).join('')}</tr></thead><tbody id="tableBody">${rows.map(row=>rowHtml(row)).join('')}</tbody></table></div>`}
